...
func (h *RubyGemsHandler) loadGemVersions(source, gemQuery string) ([]*db.Gem, error) {
...
func (h *RubyGemsHandler) GetIndex(c *gin.Context) {
	s := strings.Split(c.FullPath(), "/")
	l := len(s)
	c.File(filepath.Join(h.cfg.Dir, s[l-1]))
}

func (h *RubyGemsHandler) GetGemDependencies(c *gin.Context) {
	...
	gemVersions, err := h.loadGemVersions(h.cfg.PrivateGemsNamespace, gemQuery)
...
func (h *RubyGemsHandler) GetGemDependenciesJSON(c *gin.Context) {
	gemQuery := c.Query("gems")
	log.Trace().Str("detail", gemQuery).Msg("received gems")
	if gemQuery == "" {
		c.Status(http.StatusOK)
...
func (h *RubyGemsHandler) UploadGem(c *gin.Context) {
	var bodyBytes []byte
...
func (h *RubyGemsHandler) YankGem(c *gin.Context) {
	g := c.Query("gem")
	v := c.Query("version")
	p := c.Query("platform")
	if g == "" || v == "" {
...
func (h *RubyGemsHandler) GetGemVersionsCompact(c *gin.Context) {
...
func (h *RubyGemsHandler) GetGemNames(c *gin.Context) {
	names := h.db.GetAllGemNames(h.cfg.PrivateGemsNamespace)
	c.String(http.StatusOK, (strings.Join(names, "\n") + "\n"))
}

func (h *RubyGemsHandler) GetGemInfo(c *gin.Context) {
	gem := c.Param("gem")
	if gem == "" {
